...
SYSTEM = (
    ...
    "SECOND CHANNEL -- carveteran (@thecarveteran):\n"
    "- It is `enabled: false` ON PURPOSE and must stay that way until the user says "
    "otherwise. Both YouTube and Instagram are now fully authorised for it.\n"
    "- ** IT IS NOT A REDDIT-STORY CHANNEL. ** The concept is a grizzled veteran muscle "
    "car and a naive rookie, two-voice dialogue, teaching money-saving CAR TIPS -- facts "
    "and hacks, not stories. Its videos come from dialogue_video.py "
    "(--topic/--episode/--footage), which is built but has never been run.\n"
    "- NEVER run `bot.py --account carveteran`. bot.py generates Reddit-style stories, so "
    "pointing it at this channel produces exactly the wrong content -- that mistake was "
    "already made once on 2026-08-08 and the render was discarded. If the user asks to "
    "test the car channel, use dialogue_video.py.\n\n"
